...
impl MonsterBalancer {
    ...
    pub fn generate_monsters_for_room(
        ...
        for _ in 0..room_size {
            if let Some(monster) = self.generate_monster_for_room(room_data) {
    ...
    fn generate_monster_for_room(&self, room_data: &RoomData) -> Option<Box<dyn Monster>> {
        ...
        let monster_pool = self.get_monster_pool(&room_data.zone);
        let monster_name = monster_pool.choose(&mut rng)?;

        let monster_level = self.calculate_monster_level(room_data);
        create_monster(monster_name, monster_level)
    }

    // fn generate_boss(&self, zone: &Zone) -> Option<Box<dyn Monster>> {
    //     let boss_level = self.calculate_boss_level(zone);
    ...
    fn get_monster_pool(&self, zone: &Zone) -> Vec<String> {
        ...
        for monster_name in &zone.unique_monsters {
    ...
    fn calculate_monster_level(&self, room_data: &RoomData) -> i32 {
        let (min_level, max_level) = room_data.zone.get_monster_level_range();
        let base_level = (min_level + max_level) / 2;
        ...
        let room_bonus = match room_data.room_type {
            ...
            RoomType::Elite => 2,
            RoomType::Treasure => 1,
            RoomType::Boss => 5, // Boss plus haut niveau
        };

        (base_level + room_bonus).clamp(min_level, max_level)
    }

    fn calculate_boss_level(&self, zone: &Zone) -> i32 {
        let (_, max_level) = zone.get_monster_level_range();
        max_level + 2 // Boss plus fort que les monstres normaux
    }
}
